...
def whiten(
    ...
    p.line(np.arange(len(tdw[0].numpy())), tdw[0].numpy(), legend_label="scipy", line_color="blue")
        
    # Specify the output file and save the plot
    output_file("./py_ml_data/whiten_tf_asd.html")
    save(p)
    """
    
    #timeseries = tfs.detrend(timeseries, type=detrend, overwrite_data=True)
